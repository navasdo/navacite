...
def handle_compliance_check():
    ...
    try:
        ...
        # FIX: The model name has been updated to the latest stable version.
...
def handle_generate_note():
    ...
    try:
        api_key = app.config['GEMINI_API_KEY_SLP']
        # FIX: The model name has been updated here as well for consistency.
        google_api_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key={api_key}"
...
def handle_check_solution():
    ...
    try:
        ...
        # FIX: Also updating the model for your other AI tools to prevent future errors.
        google_api_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key={api_key}"
...
def handle_get_scaffolding():
    ...
    try:
        # Securely uses the COGNITION key from your app config
        api_key = app.config['GEMINI_API_KEY_COGNITION']
        # FIX: Updating the model here as well.
        google_api_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key={api_key}"
